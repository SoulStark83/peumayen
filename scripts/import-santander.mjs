#!/usr/bin/env node
/**
 * Importa movimientos del extracto de Santander (.xls) a la tabla `items`.
 *
 * Uso:
 *   node scripts/import-santander.mjs <fichero.xls>            # dry-run, no toca DB
 *   node scripts/import-santander.mjs <fichero.xls> --commit   # inserta
 *
 * Requiere en .env.local: NEXT_PUBLIC_SUPABASE_URL y SUPABASE_SERVICE_ROLE_KEY.
 *
 * Convenciones:
 *   - type=transaction, scope=couple, source=santander-hist
...
import { RULES, classify, prettyMerchant } from "./category-rules.mjs";
...
if (!filePath) {
  console.error("Uso: node scripts/import-santander.mjs <fichero.xls> [--commit]");
  process.exit(1);
}
...
// Header está en la fila 7 (índice): "FECHA OPERACIÓN", "FECHA VALOR", "CONCEPTO", "IMPORTE EUR", "SALDO"
const dataRows = rows.slice(8).filter((r) => r[0] && r[2] != null && r[3] != null);
...
const toInsert = [];
const statsByCat = new Map(); // "cat/sub" -> { count, total }
const unclassified = []; // muestras
let skippedDup = 0;

for (const r of dataRows) {
  const [fechaOp, fechaValor, concepto, importeRaw, saldoRaw] = r;
  const due_at = parseDate(fechaOp);
  const value_date = parseDate(fechaValor ?? fechaOp);
  ...
  const merchant = prettyMerchant(concepto);
  const key = `${value_date}|${amount}|${concepto}`;
  ...
  if (existingKeys.has(`${value_date}|${amount}|${concepto}`)) {
    skippedDup++;
    continue;
  }
